...
index_ok:  # Index is ok
    li a7, 0  # Set a7 to 0
    bge t2, a7, index_ok_2  # Check if index >= 0
    li a7, 93  # RARS syscall: Exit2
    li a0, 42  # Load exit code
    ecall  # Call exit
...
assert_true:
    ...
    # Index begin
    li s2, 1
    blt s2, s5, index_ok_3  # Check if index less then length
    li a7, 93  # RARS syscall: Exit2
    li a0, 42  # Load exit code
    ecall  # Call exit
index_ok_3:  # Index is ok
    li a7, 0  # Set a7 to 0
    bge s2, a7, index_ok_4  # Check if index >= 0
    li a7, 93  # RARS syscall: Exit2
    li a0, 42  # load exit code
    ecall  # Call exit
index_ok_4:  # Index is ok
    ...
    lw s5, 4(s1)  # Copying array length to target register + 4
    li s1, 1
    blt s1, s5, index_ok_6  # Check if index less then length
    li a7, 93  # RARS syscall: Exit2
    li a0, 42  # Load exit code
    ecall  # Call exit
index_ok_6:  # Index is ok
    li a7, 0  # Set a7 to 0
    bge s1, a7, index_ok_7  # Check if index >= 0
    li a7, 93  # RARS syscall: Exit2
    li a0, 42  # Load exit code
    ecall  # Call exit
...
eq_end_9:
    addi s1, s1, -1
    beqz s1, assert_true_5  # Jump if assertion OK
    li a7, 93  # RARS syscall: Exit2
    li a0, 42  # Assertion violation exit code
    ecall
assert_true_5:
    ...
    lw s5, 4(s1)  # Copying array length to target register + 4
    li s1, 0
    blt s1, s5, index_ok_11  # Check if index less then length
    li a7, 93  # RARS syscall: Exit2
    li a0, 42  # Load exit code
    ecall  # Call exit
index_ok_11:  # Index is ok
    li a7, 0  # Set a7 to 0
    bge s1, a7, index_ok_12  # Check if index >= 0
    li a7, 93  # RARS syscall: Exit2
    li a0, 42  # Load exit code
    ecall  # Call exit
index_ok_12:  # Index is ok
    li t3, 4  # Load the size of each element in the array
    mul s1, s1, t3  # Calculate the offset (index) from the base address
    add s1, s1, s2  # Compute array element address
    lw s1, 0(s1)  # Load array element
    li s2, 3
    beq s1, s2, eq_true_13
    li s1, 0  # Comparison result is false
    j eq_end_14
eq_true_13:
    li s1, 1  # Comparison result is true
eq_end_14:
    addi s1, s1, -1
    beqz s1, assert_true_10  # Jump if assertion OK
    li a7, 93  # RARS syscall: Exit2
    li a0, 42  # Assertion violation exit code
    ecall
assert_true_10:
