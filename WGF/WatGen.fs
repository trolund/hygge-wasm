...
let rec genWat (instrs: Wasm Commented  list) (ident: int) =
        ...
        let rec aux (instrs: Commented<Instr.Wasm> list) (watCode: string) (indent: int) =
            ...
            | head :: tail ->
                ...
                | I32Add instrs when style = Folded -> 
                    let watCode = watCode + space + $"({instrLabel instr}{commentS c}\n{genWat instrs (indent + 1)}{gIndent (indent)})\n"
                ...
                | I32Add instrs when style = Linar -> 
                    aux tail (watCode + (genWat instrs indent) + $"{gIndent indent}{instrLabel instr}{commentS c}\n" ) indent
                | _ ->

                    let f i = if style = Folded then $"({i})" else i
                    // generate wat code for plain unested instr
                    let instrAsString =
                        (printInstr head) + if (c.Length > 0) then $" ;; %s{c}\n" else "\n" in
    ...
    and printInstr (i: Commented<Instr.Wasm>) =
            ...
            let isComment = match instr with | Comment _ -> true | _ -> false
